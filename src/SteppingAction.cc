...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
    ...
    //MODIFY - TRY TO CHANGE/DELETE SOME OF THE CONDITIONS; COMPARE THE NAMES OF VOLUMES WITH
    //THEIR NAMES in DetectorConstruction, TRY TO MODIFY
    if (step->GetPreStepPoint()-> GetStepStatus()==G4StepStatus::fGeomBoundary&&
        (volumeName=="Target"||volumeName=="Detector"))
    {

        //coordinates
        //MODIFY - CHANGE THE UNITS
        ...
        G4double angle_x = std::atan(momentumDirection.getX()/momentumDirection.getZ());
        G4double angle_y = std::atan(momentumDirection.getY()/momentumDirection.getZ());

        //kinetic energy
        //MODIFY - CHANGE THE UNIT
        ...
        //STUDY THE STRUCTURE AND THE VALUES; COMPARE IT WITH RunAction, TRY TO MODIFY
